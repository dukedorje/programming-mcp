...
function getPersonaCategory(persona: any): string {
  ...
  if (expertiseStr.includes('architecture') || expertiseStr.includes('system design')) {
    return 'architecture';
  }
  if (expertiseStr.includes('algorithm') || expertiseStr.includes('optimization')) {
    return 'algorithms';
  }
  if (expertiseStr.includes('security') || expertiseStr.includes('authentication')) {
    return 'security';
  }
  if (expertiseStr.includes('frontend') || expertiseStr.includes('react') || expertiseStr.includes('ui')) {
    return 'frontend';
  }
  if (expertiseStr.includes('backend') || expertiseStr.includes('api') || expertiseStr.includes('database')) {
    return 'backend';
  }
